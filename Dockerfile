...
RUN echo 'APT::Get::Assume-Yes "true";' > /etc/apt/apt.conf.d/90test \
  && echo 'DPkg::Options "--force-confnew";' >> /etc/apt/apt.conf.d/90test

# Make sure PATH includes ~/.local/bin
# https://bugs.debian.org/cgi-bin/bugreport.cgi?bug=839155
# This only works for root. The test user is done near the end of this Dockerfile
...
RUN groupadd --gid 3434 test \
  && useradd --uid 3434 --gid test --shell /bin/bash --create-home test \
  && echo 'test ALL=NOPASSWD: ALL' >> /etc/sudoers.d/50-test \
  && echo 'Defaults    env_keep += "DEBIAN_FRONTEND"' >> /etc/sudoers.d/env_keep

USER test
ENV PATH /home/test/.local/bin:/home/test/bin:${PATH}
...
LABEL io.test.preserve-entrypoint=true
